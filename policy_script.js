...
const USER_NAME_BADGE_SUBTITLE = 'Доступ активирован';
...
const PREMIUM_ACCESS_STORAGE_KEY = 'quizPremiumActivationGlobal_v2';
const PREMIUM_ACTIVATION_PUBLIC_KEY_PEM = `-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEm9JB/u6RGN3pUraBVn6IPeJwNWcI
+mUN/CsvUfHUuEfmqgjLKPHdGD8xm8BP9JFhtJ0rHkLL86Oxf4Ud/rR9FQ==
-----END PUBLIC KEY-----`;
...
const QUIZ_TEST_PAGE_BY_NAMESPACE = {
  macro: 'macro_test.html',
  policy: 'policy_test.html',
  money: 'money_test.html'
};
  ...
  if (value && typeof value === 'object' && value.users && typeof value.users === 'object') {
    Object.entries(value.users).forEach(([rawUserId, rawRecord]) => {
      ...
      users[userId] = {
        ...
        version: String(rawRecord?.version || 'v1'),
...
function getStoredPremiumActivationStore() {
  try {
    return normalizePremiumActivationStore(JSON.parse(localStorage.getItem(PREMIUM_ACCESS_STORAGE_KEY) || '{}'));
  } catch (_) {
...
function hasStoredPremiumAccessForUser(userId) {
...
function grantPremiumAccessForUser(userId, details = {}) {
  ...
  store.users[normalizedUserId] = {
    ...
    version: String(details?.version || 'v1'),
...
function parsePremiumActivationToken(token) {
  ...
  if (!version || !userId || !normalizedDate || !normalizedNonce) return null;
  ...
  if (!document.getElementById('ban-overlay')) {
    ...
    banOverlay.innerHTML = `
      <div class="agreement-panel">
        ...
        <h2 class="agreement-title">Введите ключ для активации премиум-доступа</h2>
        <p class="agreement-lead" data-role="activation-lead">Ключ создаётся по вашему Telegram ID и действует только в день выдачи.</p>
    ...
    const submitActivation = async () => {
      ...
      notifyPremiumActivationToTelegram({ userId });

      if (noteNode) {
        noteNode.textContent = 'Премиум-доступ успешно активирован.';
...
function applyPremiumAccessOverlayContent(accessStatus = {}) {
  ...
  if (leadNode) {
    leadNode.textContent = normalizedUserId
      ? 'Введите ключ для активации доступа. Ключ действует только в день выдачи и привязан к вашему Telegram ID.'
      : 'По вопросам активации премиум-доступа: @SayfiddinovM · Sayfiddinov™.';
  }

  if (noteNode) {
    noteNode.textContent = normalizedUserId
      ? 'После успешной активации доступ остаётся навсегда на этом устройстве.'
...
async function getCurrentBanStatus() {
  ...
  const hasAccess = normalizedUserId ? hasStoredPremiumAccessForUser(normalizedUserId) : false;
...
function getDefaultQuizThemeFile() {
  const defaults = {
    macro: 'macro_tests.json',
    policy: 'policy_tests.json',
    money: 'money_tests.json'
  };

  return defaults[QUIZ_STORAGE_NAMESPACE] || 'policy_tests.json';
}
...
async function notifyPremiumActivationToTelegram({ userId } = {}) {
  ...
  try {
    ...
    const message = [
      '🔐 Пользователь активировал ключ премиума',
      '',
      `Имя: ${displayName}`,
      `Username: ${username}`,
      `ID: ${normalizedUserId}`,
      `Раздел: ${QUIZ_STORAGE_NAMESPACE}`,
      `Дата: ${activatedAt}`
    ].join('\n');
    ...
    console.error('Не удалось отправить уведомление об активации премиума в Telegram:', error);
    return false;
  }
}
...
function getQuizTestPagePath() {
  return QUIZ_TEST_PAGE_BY_NAMESPACE[QUIZ_STORAGE_NAMESPACE] || `${QUIZ_STORAGE_NAMESPACE}_test.html`;
}
